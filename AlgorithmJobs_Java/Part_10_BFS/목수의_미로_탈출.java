...
public class 목수의_미로_탈출{
    ...
    static void BFS(int y, int x, int distance[][]){
        ...
        Queue<Pair1> queue = new LinkedList<>();   //큐 생성
        boolean visited[][] = new boolean[MAX][MAX];           //정점 방문 체크

        queue.add(new Pair1(y,x));  //(시작 y좌표, 시작 x좌표)를 큐에 삽입
        ...
        while(queue.size() > 0){   //큐의 크기만큼 반복
            ...
            for(int j=0; j<dy.length; j++){    //상하좌우 만큼 한칸 이동
                ...
                if(nrow < n && nrow >= 0 && ncol < m && ncol>=0 && visited[nrow][ncol]==false){  //row와 col 값이 배열안에 있으면              /&& arr[nrow][ncol]==0 ... 1이여도 방문할수 있게함.
                    ...
                    if(arr[nrow][ncol]==0){   //벽을 만나지 않았다면
                        queue.add(new Pair1(nrow,ncol));   //(nrow,ncol) 정점을 큐에 삽입
                    }
                }
